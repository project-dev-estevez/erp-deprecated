    ...
    <div class="page <?php echo $clase_pagina ?>">
      ...
      <table style="width:100%;font-size: 8px;" border="1" cellpadding="4" cellspacing="0">
        <tbody>
          ...
          <tr>
            <td style="text-align: left" colspan="2"><strong>Requisición: </strong> <?= $pedido->tbl_solicitudes_estimacion_idtbl_solicitudes_estimacion ?></td>
      ...
      <table style="font-size:8px;" class="table table-border" border="1" cellspacing="0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="text-align: center;"><?= $value->cantidad_contrato ?></td>
          ...
          <?php endforeach ?>
          <tr>
            <td style="text-align: left;" colspan="3" rowspan="9"><strong>Nota del pedido:</strong> <?= $pedido->observaciones ?></td>
            <th colspan="3" style="text-align: left;">Importe de la estimación"</th>
          ...
          <tr>
            <th colspan="3" style="text-align: left;">Retención de fondo de garantia (<?= $pedido->porcentaje_fondo_retencion_garantia ?>%):</th>
          ...
          <tr>
            <th colspan="3" style="text-align: left;">IVA <?= $pedido->iva === NULL ? '16' : $pedido->iva ?>%</th>
              <td style="text-align: left;">$<?= number_format($subtotal_estimacion*($pedido->iva/100),2) ?></td>
          </tr>
          ...
          <?php } 
          $total = ($subtotal_estimacion*($pedido->iva/100))+$subtotal_estimacion-($subtotal_estimacion * ($pedido->porcentaje_retencion/100));
          ?>
          <?php if($pedido->amortizacion != 0){ ?>
          <tr>
            <th colspan="<?= $pedido->estimacion == 0 ? "1" : "3" ?>" style="text-align: left;">Descuento</th>
            <td style="text-align: left;">$<?= number_format($pedido->amortizacion,2) ?></td>
          </tr>
          <?php 
          ...
          <tr>
            <th colspan="3" style="text-align: left;">Nota Credito</th>
            <td style="text-align: left;">$<?= number_format($pedido->nota_credito,2) ?></td>
          </tr>
          <tr>
            <th colspan="3" style="text-align: left;">Total</th>
              <td style="text-align: left;">$<?= number_format($total - $pedido->nota_credito,2) ?></td>
          </tr>
          <tr>
            <?php if($pedido->tipo_moneda == 'p') { ?>
              <?php if(($pedido->iva !== 0 && $pedido->iva !== NULL) || $pedido->iva === NULL) { ?>
                <td style="text-align: right;" colspan="9" align="right">(* <?= numletras(number_format($total - $pedido->nota_credito, 2),'p') ?> *)</td>
              <?php } ?>
            <?php } ?>
            <?php if($pedido->tipo_moneda == 'd') { ?>
              <?php if(($pedido->iva !== 0 && $pedido->iva !== NULL) || $pedido->iva === NULL) { ?>
                <td style="text-align: right;" colspan="9" align="right">(* <?= numletras(number_format($total - $pedido->nota_credito, 2),'d') ?> *)</td>
              <?php } ?>
            <?php } ?>
          </tr>
        </tbody>
