  ...
  <div class="container-fluid">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <?php echo form_open_multipart(base_url().'almacen/guardar-nueva-explosion-insumos', 'class="grid-form needs-validation form" novalidate'); ?>
        <input type="hidden" name="uid_almacen" value="<?= $uid_almacen; ?>">
        <fieldset>
          <legend>Detalles de entrada</legend>
          <fieldset id="almacenFieldset" class="proyectoFieldset">
          <div data-row-span="6">
            <div data-field-span="3">            
              <label>Almacen*</label>
              <select name="almacen_origen" id="almacen_origen" class="selectpicker proyecto" required data-live-search="true">
                <option value="" disabled="disabled" selected="selected">Seleccione...</option>
               
                  <option value="1">Almacén General</option>
        
                  <!--<option value="2">Almacén Alto Costo</option>-->
                  <option value="16">Almacén Kuali</option>

              </select> 
            </div>
            <div data-field-span="2">            
              <label>Segmento*</label>
              <select name="segmento" id="segmento" class="form-control segmento" required data-live-search="true">
                <option value="" disabled="disabled" selected="selected">Seleccione...</option>                
                  <?php foreach($listadoSegmentos AS $key => $value){ ?>
                    <?php if($value->direccion){ ?>
                      <option value=""><?= $value->direccion ?></option>
                    <?php }else{ ?>
            ...
            <div data-field-span="1">            
              <label>Tipo*</label>
              <select name="tipo_explosion" id="tipo_explosion" class="form-control segmento" required>
                <option value="" disabled="disabled" selected="selected">Seleccione...</option>                
        ...
        <?php if($proyecto_almacen[0]->tbl_clientes_idtbl_clientes != 11 && $proyecto_almacen[0]->tbl_clientes_idtbl_clientes != 28 && $proyecto_almacen[0]->tbl_clientes_idtbl_clientes != 47){ ?>
        ...
        <div class="clearfix pt-md">
          <div class="pull-right">
            ...
            <?= form_hidden('cliente', $proyecto_almacen[0]->tbl_clientes_idtbl_clientes) ?>
...
<script>
  /*$(document).on('change', '.productoo', function(event) {
      ...
      <?php } else { ?>
        if($("option:selected", this).data("categoria") == 7 || $("option:selected", this).data("categoria") == 10 || $("option:selected", this).data("categoria") == 16 || $("option:selected", this).data("categoria") == 17 || $("option:selected", this).data("idtblcatalogo") == 19420 || $("option:selected", this).data("idtblcatalogo") == 19421 || $("option:selected", this).data("idtblcatalogo") == 19428 || $("option:selected", this).data("idtblcatalogo") == 19429 || $("option:selected", this).data("idtblcatalogo") == 24792 || $("option:selected", this).data("idtblcatalogo") == 24793 || $("option:selected", this).data("idtblcatalogo") == 25053){
          $(_this).find('.cantidad').attr('readonly', 'readonly').val('1');
          $(_this).find('.campos_ac').show(500);
          $(_this).find('.campos_ac').find('input').removeAttr('disabled').attr('required', 'required');
        } else {
...
<script>
  ...
  $(document).on('change', '#tipo_explosion', function (event) {
    event.preventDefault();
    $("#productos_cliente").html('');
    $.ajax({
      type:"POST",
      url:base_url+'almacen/getMaterialesClientes',
      data:'categoria=' + $(this).val() + '&cliente='+11,
      success:function(r) {  
        var registros = eval(r);
        var index = 0;
        for(i = 0; i < registros.length; i++) {
          index = i+1;
          $('#productos_cliente').append('<fieldset id="productoFieldset'+index+'" class="productoFieldset"><div data-row-span="5"><div data-field-span="3"><label>Producto*</label><input class="form-control neodata" autocomplete="off" value="'+'('+registros[i].neodata+')'+registros[i].descripcion+'" type="text" required><input class="form-control producto" autocomplete="off" value="'+registros[i].idtbl_catalogo+'" type="hidden" name="producto[]" required><div class="list-group sugerencias"></div><input type="hidden" name="categoria[]" class="categoria" value="'+registros[i].ctl_categorias_idctl_categorias+'"></div><div data-field-span="1"><label>Cantidad</label><input type="number" step="0.0001" name="cantidad[]" value="0.0000" class="cantidad" required></div><div data-field-span="1"><label>Unidad</label><input type="text" name="unidad_medida[]" class="cantidad" value="'+registros[i].unidad_medida_abr+'" required></div></div>');
        }
      }
    })
  });
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            $.each(data, function(key, item) {
                filas += "<div><a class='elemento-sugerido list-group-item' data-idtblcatalogo='" + item.idtbl_catalogo + "' data='" + item.neodata + " " + item.marca + " " + item.modelo + " (" + item.descripcion.substr(0,60) + ")" + "' data-precio='" + item.precio + "' data-moneda='" + item.tipo_moneda +"' data-preciolabel='" + parseFloat(item.precio).toFixed(2) + (item.tipo_moneda == "p" ? ' Pesos' : ' Dolares') + "' data-categoria='" + item.ctl_categorias_idctl_categorias + "'>" + item.neodata + " " + item.marca + " " + item.modelo + " (" + item.descripcion.substr(0,60) + ")" + "</a></div>";
            });
