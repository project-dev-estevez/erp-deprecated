                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <select name="reembolsado" id="reembolsado" class="form-control" required>
                                <option value="" disabled="disabled" selected="selected">Seleccione...</option>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <ul class="nav nav-tabs">
              <li class="nav-item">
                <a class="nav-link btn active" data-toggle="pill" href="#tabCajaChica" role="tab" aria-selected="true">Caja Chica</a>
              </li>
            </ul>
            ...
            <div class="tab-content">              
              <div class="tab-pane active" id="tabCajaChica">
                <br/><br/>
                <a href="#caja_chica_modal" data-toggle='modal' data-action='new' class="btn btn-primary">Comprobación de Gastos</a>
                <?php if($this->session->userdata('tipo') == 3){ ?>
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="caja_chica_contable">
                    ...
                    <div class="text-left">
                      <br><br>
                      <h4>Total Comprobaciones:<span id="total_comprobado_contable" style="font-weight: normal;"></span></h4>
                    </div>
                    ...
                    <div class="text-right">
                      <br><br>
                      <h4>Caja Actual:<span class="total_adeudo" style="font-weight: normal;"></span></h4>
                    </div>
                    
                    <div class="text-right">
                      <br><br>
                      <h4>Total Comprobado:<span id="total_caja_chica_contable" style="font-weight: normal;"></span></h4>
                      <input type="hidden" id="total_caja_chica_contable_input">
                    </div>

                    <div class="text-right">
                      <br><br>
                      <h4>Por Comprobar:<span class="por_comprobar" id="por_comprobar_contable" style="font-weight: normal;"></span></h4>
                    </div>
                  ...
                  <div class="tab-pane fade" id="caja_chica_no_contable">
                    ...
                    <div class="text-left">
                      <br><br>
                      <h4>Total Comprobaciones:<span id="total_comprobado_no_contable" style="font-weight: normal;"></span></h4>
                    </div>
                    <div class="text-right">
                      <span class="form-group text-left">
                        <label>Fecha Incial</label>
                    ...
                    <div class="text-right">
                      <br><br>
                      <h4>Total Comprobado:<span id="total_caja_chica_no_contable" style="font-weight: normal;"></span></h4>
                      <input type="hidden" id="total_caja_chica_no_contable_input">
                    </div>
                    <div class="text-right">
                      <br><br>
                      <h4>Por Comprobar:<span class="por_comprobar" id="por_comprobar_no_contable" style="font-weight: normal;"></span></h4>
                    </div>
                    <div class="table-responsive">
...
function mostrarDatosCajaChica(fecha_inicial, fecha_final, pagina, tipo) {
    ...
    var ciclo = '<?= $ciclo ?>';
  $.ajax({
    url: "<?= base_url() ?>Almacen/obtener_caja_chica_solicitud",
      type: "POST",
      data: {
      ...
      success: function(response) {
        ...
        $.each(response.caja_chica, function(key, item) {
            ...
            if(item.confirmacion == 2) {
              ...
              estatus = 'Finalizado';
            } else if(item.confirmacion == 1 && item.confirmacion_direccion == 0) {
              className = 'table-primary';
              estatus = 'Pendiente Aprobación Dirección';
            } else if(item.confirmacion == 3){
              className = 'table-danger';
            } else if(item.confirmacion_direccion == 1){
              className = 'table-info';
              estatus = 'Pendiente Aprobación Contabilidad';
            } else {
            ...
            if(item.reembolsado == '1'){
              filas += "<td style='color: red; font-weight: bold;'>" + "Si" + "</td>";
            }else{
              filas += "<td style='color: red; font-weight: bold;'>" + "No" + "</td>";
            }
            filas += "<td>" + estatus + "</td>";
            ...
            if(item.confirmacion == 3){
              ...
              <?php } elseif($this->session->userdata('id') == 68){ ?>
                filas += "<a href='<?= base_url() ?>uploads/caja_chica/" + item.uid + ".pdf' target='_black'><i class='fa fa-file-pdf-o'></i></a>";
                if(item.confirmacion == 1 && item.confirmacion_direccion == 0){
                  filas += "<a href='#' id='confirmacion_movimiento_caja_chica_" + tipo + "' data-idtbl-caja-chica='"+ item.idtbl_caja_chica  +"'><i class='fa fa-check' aria-hidden='true'></i></a>";
                  filas += "<a href='#' id='cancelar_movimiento_caja_chica_" + tipo + "' data-idtbl-caja-chica='"+ item.idtbl_caja_chica  +"'><i class='fa fa-times' aria-hidden='true'></i></a>";
                  //filas += "<a href='#' id='reembolsar_movimiento_caja_chica_" + tipo + "' data-idtbl-caja-chica='"+ item.idtbl_caja_chica  +"'><i class='fa fa-refresh' aria-hidden='true'></i></a>";
                }
                <?php } elseif($this->session->userdata('tipo') == 16){ ?>
                filas += "<a href='<?= base_url() ?>uploads/caja_chica/" + item.uid + ".pdf' target='_black'><i class='fa fa-file-pdf-o'></i></a>";
                if(item.confirmacion == 1 && item.confirmacion_direccion == 1){
                  filas += "<a href='#' id='confirmacion_movimiento_caja_chica_" + tipo + "' data-idtbl-caja-chica='"+ item.idtbl_caja_chica  +"'><i class='fa fa-check' aria-hidden='true'></i></a>";
                  filas += "<a href='#' id='cancelar_movimiento_caja_chica_" + tipo + "' data-idtbl-caja-chica='"+ item.idtbl_caja_chica  +"'><i class='fa fa-times' aria-hidden='true'></i></a>";
                }
              <?php } else{ ?>
        ...
        if(tipo == "contable"){
          $("#total_caja_chica_contable").text(" $ " + parseFloat(response.monto).toFixed(2));
          $("#total_comprobado_contable").text(" $ " + parseFloat(response.monto_total).toFixed(2));
          $("#total_caja_chica_contable_input").val(parseFloat(response.monto).toFixed(2))
          var contable = $("#total_caja_chica_contable_input").val();
        }else{
          $("#total_caja_chica_no_contable").text(" $ " + parseFloat(response.monto).toFixed(2));
          $("#total_comprobado_no_contable").text(" $ " + parseFloat(response.monto_total).toFixed(2));
          $("#total_caja_chica_no_contable_input").val(parseFloat(response.monto).toFixed(2));
          var no_contable = $("#total_caja_chica_no_contable_input").val();
        }
        var total = contable + no_contable;
       
        $('.total_adeudo').text(" $" + parseFloat(response.caja_chica[0].cantidad).toFixed(2));
        if(tipo == "contable"){
        $('#por_comprobar_contable').text(" $" + parseFloat(parseFloat(response.caja_chica[0].cantidad).toFixed(2) - parseFloat(response.monto).toFixed(2)).toFixed(2));
        }else{
          $('#por_comprobar_no_contable').text(" $" + parseFloat(parseFloat(response.caja_chica[0].cantidad).toFixed(2) - parseFloat(response.monto).toFixed(2)).toFixed(2));
...
$("body").on("click", "#reembolsar_movimiento_caja_chica_contable", function(e){
  e.preventDefault();
  var button = $(this);
  var recipient = button.data();
  var pagina = parseInt($(".paginacioncajachica_contable").find(".active a").text());
  reembolsar_movimiento(recipient, pagina, "contable");
});
...
function reembolsar_movimiento(recipient, pagina, tipo){
  Swal.fire({
    title: '¿Deseas reembolsar el movimiento?',
    text: "Cofirmación movimiento",
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
    ...
    if (result.value) {
      $.ajax({
        url: "<?= base_url() ?>ControlVehicular/reembolsar_movimiento_caja_chica",
        type: "POST",
        data: {
          idtbl_caja_chica: recipient.idtblCajaChica,
          token: '<?= $token ?>'
...
$("#caja_chica_form").on("submit", function(e){
    ...
    var form = $(this);
    <?php if($this->session->userdata('tipo') == 3){ ?>
            var controlador = 'ControlVehicular';
        <?php }else{ ?>
    ...
    $.ajax({
        url: "<?= base_url() ?>"+controlador+"/registro_caja_chica",
        type: "post",
        data: formData,
        processData: false,
        contentType: false,
