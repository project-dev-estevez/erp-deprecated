  ...
  <div class="container-fluid">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <?php echo form_open_multipart(base_url().'almacen/guardar-nueva-entrada-alto-costo', 'class="grid-form needs-validation" novalidate'); ?>
                ...
                <div data-field-span="4">
                  ...
                  <select name="producto[]" class="selectpicker producto" required data-live-search="true">
                    <?php foreach ($catalogo as $subkey => $subvalue): ?>
                      ...
                      data-categoria="<?php echo $subvalue->idctl_categorias ?>"><?php echo $subvalue->uid.' '.$subvalue->marca.' '.$subvalue->modelo.' ('.$subvalue->descripcion.')' ?>
                      </option>
                    <?php endforeach ?>
                  </select>
              ...
              <div data-field-span="4">
                ...
                <select name="producto[]" class="selectpicker producto" required data-live-search="true">
                  ...
                  <?php foreach ($catalogo as $key => $value): ?>
                    ...
                    data-categoria="<?php echo $value->idctl_categorias ?>"><?php echo $value->neodata.' '.$value->marca.' '.$value->modelo.' ('.$value->descripcion.')' ?></option>
                  <?php endforeach ?>
                </select>
...
<script>
  $(document).on('change', '.producto', function(event) {
    ...
    if($("option:selected", this).data("categoria") == <?= ID_ACTIVO_FIJO ?> || $("option:selected", this).data("categoria") == <?= ID_HERRAMIENTA_HILTI ?> || $("option:selected", this).data("categoria") == <?= ID_HERRAMIENTA ?>  ){
