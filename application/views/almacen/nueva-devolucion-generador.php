<section class="forms nueva-asignacion">
  ...
  <div class="card">
    <div class="card-header d-flex align-items-center">
      <h3 class="h4">Nueva Solicitud de Devolución Almacén General</h3>
    </div>
    <div class="card-body">
      <div class="over"></div>
      ...
      <?= form_open_multipart('', 'class="needs-validation" novalidate id="form-asignacion"'); ?>
        ...
        <div class="row">
          ...
          <?php if($this->session->userdata('tipo') == 5){ ?>
            <div class="col" style="display: none;" id="select_estatus">
              <div class="form-group">
                <label>Estatus</label>
                <select name="estatus_personal" id="estatus_personal" class="form-control" required>
                  <option value="" disabled selected>Seleccione...</option>
            ...
            <div class="col-6">
              ...
              <select name="producto[]" id="product" class="selectpicker producto" required data-live-search="true">
                ...
                <?php foreach ($catalogo as $key => $value): ?>
                  <?php if (($value->tipo_producto == 1 || $value->tipo_producto == null || $value->tipo_producto == '') && $this->session->userdata('tipo') == 5): ?>
                    <option value="<?php echo $value->idtbl_catalogo ?>"
                      ...
                      <?php echo $value->neodata.' '.$value->marca.' '.$value->modelo.' ('.substr(htmlspecialchars(trim($value->descripcion), ENT_COMPAT), 0, 30).')' ?>
                    </option>
                  <?php elseif (($value->tipo_producto == 1 || $value->tipo_producto == null || $value->tipo_producto == '') && $this->session->userdata('tipo') != 5): ?>
                    <option value="<?php echo $value->idtbl_catalogo ?>"
                      data-descripcion="<?php echo htmlspecialchars(trim($value->descripcion), ENT_COMPAT) ?>"
                      data-unidad-medida="<?php echo $value->unidad_medida ?>"
                      data-categoria="<?php echo $value->idctl_categorias ?>">
              ...
              <label>Producto*</label>
              <input class="form-control neodata" autocomplete="off" type="text" required value="<?= $material->descripcion ?>" disabled>
              <input class="form-control producto" autocomplete="off" type="hidden" name="producto[]" required value="<?= $material->tbl_catalogo_idtbl_catalogo ?>">
              <input class="form-control producto" autocomplete="off" type="hidden" name="iddtl_asignacion[]" required value="<?= $material->iddtl_asignacion ?>">
              <div class="list-group sugerencias"></div>
            </div>
            <div class="col">
              <label for="cantidad">Cantidad*</label>
              <input type="number" name="cantidad[]" id="cantidad" required class="form-control cantidad" step="0.001" value="<?= $material->cantidad ?>">
            </div>
            <div class="col">
              <label for="unidad">Unidad</label>
              <input type="text" disabled="true" class="form-control unidad" value="<?= $material->unidad ?>">
            </div>
          </div>
          <br>
          <div class="row">
        ...
        <div class="text-right">
          ...
          <?= form_hidden('tipo_devolucion','Almacen General') ?>
...
$(document).on('change', '#tipo', function(event) {
  ...
  if(this.value == 'interno'){
      $("#select_estatus").show(500);
    }else{
      $("#select_estatus").hide(500);
    }
...
$(document).on('change', '#estatus_personal', function(event) {
    event.preventDefault();
    var _this=$(this);
    $('#recibe').find('option').remove().end().append('<option value="" disabled="disabled" selected="selected">Seleccione...</option>');
    $.ajax({
      url: base_url+'almacen/personal-estatus',
      type: 'POST',
      dataType: 'json',
      data: {estatus: _this.val()},
      beforeSend: function() {
        _this.closest('.card-body').addClass('load');
      },
      success : function(data) {
...
$(document).ready(function () {
  ...
  $("#form-asignacion").on("submit", function (event) {
    if (event.isDefaultPrevented()) {
      ...
      $.ajax({
        url: "<?= base_url()?>almacen/guardar-solicitud-devolucion-generador",
        type: "post",
        dataType: "json",
        data: formData,
        cache: false,
