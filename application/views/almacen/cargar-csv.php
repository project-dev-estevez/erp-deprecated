  ...
  <script>
    ...
    function procesar_csv(data) {
    	...
    	for(var row in data) {
    		...
    		console.log(row);
    		var request = $.ajax({
    			url: '<?= base_url() ?>almacen/procesar-csv',
    			type: 'POST',
    			async:true,
    			dataType: 'json',
    			data: {tipo_red: data[row][0], tramo: data[row][1], concepto: data[row][2], unidad: data[row][3], cantidad: data[row][4], lat_a: data[row][5], lon_a: data[row][6], lat_b: data[row][7], lon_b: data[row][8], proyecto: data[row][9], segmento: data[row][10], cliente: data[row][11], row:row, token: '<?= $token ?>'},
    		})
    		.done(function(data) {
    			console.log(data)
    			if(data.error===true){
    				if(data.texto=='ya_existe'){
    					total_errores_no_existe++;
    					$('#row-'+data.row).addClass('table-warning').find('.estatus').html('<strong>Tramo ya existente</stong>');
    				}
    				else if(data.texto=='no_numerico'){
    					total_filas_sin_info++;
    					$('#row-'+data.row).addClass('table-danger').find('.estatus').html('<strong>Error de carga</strong>');
    				}
    				else{
    					total_errores++;
