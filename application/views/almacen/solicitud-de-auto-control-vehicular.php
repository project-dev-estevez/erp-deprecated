  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header d-flex align-items-center">
        <h3 class="h4">Nueva Solicitud de Auto(Control Vehicular)</h3>
      </div>
      <div class="card-body">
        <div class="over"></div>
        ...
        <input type="hidden" name="tipo_producto" value="Control Vehicular">
        <hr>
        <div id="item-producto1" class="itemproducto">
          <div id="hola" class="hola">
            <div class="form-row">
              <div class="col">
                <label>Producto*</label>
                <select name="producto[]" id="product" class="selectpicker producto" required data-live-search="true">
                  <option value="" disabled="disabled" selected="selected">Seleccione...</option>
                  <option value="">Producto inexistente en el catálogo</option>
                  <?php foreach ($catalogo as $key => $value): ?>
                    <?php if($value->tipo_producto != 1 && $value->descripcion != "PERSONAL") { ?>
                    ...
                    <?php } ?>
                  <?php endforeach ?>
                  <input type="hidden" name="iddtl_almacen[]" class="form-control iddtlalmacen">
                  <input type="hidden" name="categoria[]" class="form-control categoria">
                </select>
              </div>
              <div class="col">
            ...
            <div class="campos_ac" style="display:none">
            <div class="form-row">
              <div class="col-md-4 ">
                <label>Aditamentos:</label>  
                <select name="aditamento" id="aditamento" class="selectpicker" required>
                  <option>Seleccione...</option>
                  <option value="1">Porta carrete</option>
                  <option value="2">Porta escaleras</option>
...
<script>
  ...
  $(document).on('change', '.producto', function (event) {
    ...
    $(_this).find('.iddtlalmacen').val($("option:selected", this).data("iddtl-almacen"));
    $(_this).find('.categoria').val($("option:selected", this).data("categoria"));
    var _this = $(this).closest('.itemproducto');    
    var valorSeleccionado = $(this).val();
    console.log(valorSeleccionado);
    if (valorSeleccionado == 25235 || valorSeleccionado == 25237 || valorSeleccionado == 25238 || valorSeleccionado == 26945){
      $(_this).find('.campos_ac').show(500);
      $(_this).find('.campos_ac').find('select').removeAttr('disabled').attr('required', 'required');
      ...
      console.log('Ok');      
      $(_this).find('.campos_ac').hide(500); 
      $(_this).find('#aditamento').val(('')).selectpicker('refresh');
    }
  });
    ...
    $.ajax({
      ...
      success : function(data) {
        ...
        if (_this.val() == 'interno') {
          ...
          $.each(personalAutoriza, function (i, item) {
            //if (item.idtbl_users == 17 || item.idtbl_users == 66){
              $('#persona_autorizacion').append($('<option>', {
                value: item.idtbl_users,
                text : item.nombre
              }));
  ...
  $('#nuevoProducto').click(function (event) {
    ...
    var $klon = $div.clone().prop('id', 'hola' + num);
    ...
    $('#hola' + num + ' .selectpicker').selectpicker();
  ...
  $(document).on('click', '.delete-product', function () {
    $(this).parents('div[id^="hola"]').remove();
  });
