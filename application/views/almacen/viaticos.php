            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-4 col-lg-4">
                        <div class="form-group">
                            <label>Costo</label>
                            <input type="number" placeholder="Costo" name="costo" class="form-control" step="0.0001"
                                required>
                        </div>
                    </div>
                    <div class="col-12">
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <select name="idtbl_proyectos" id="selectProyecto" class="selectpicker proyecto" required
            ...
            <div class="col">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-sm table-hover" id="tbviaticos">
                                <thead>
                                    <tr>
...
function mostrarDatos(valorBuscar, pagina, valorBuscar2) {
    $.ajax({
        url: "<?= base_url() ?>almacen/mostrarViaticos",
        type: "POST",
        data: {
            buscar: valorBuscar,
            nropagina: pagina,
        ...
        success: function(response) {
            ...
            $.each(response.viaticos, function(key, item) {
                if (item.estatus == 'creada') {
                    ...
                    clase = 'table-info';
                    porcentaje = '80%';
                } else if (item.estatus == 'aprobada_rh') {
                    estatus = 'Pendiente Aprobación Contabilidad';
                    clase = 'table-info';
                    porcentaje = '90%';
                } else if (item.estatus == 'aprobada_pm') {
                ...
                if (item.idtbl_solicitud_recursos != null) {
                    ...
                    filas += "<td>" + fecha + "</td>";
                    filas += "<td>" + item.nombre_recibe + " " + item.apellido_paterno + " " + item
                        .apellido_materno + "</td>";
                    ...
                    filas += "<td align='center'><a href='" + "<?= base_url() ?>" +
                        "almacen/detalle_recurso/" + item.uid + "/" + item.tipo +
                        "' title='Detalles'><i class='fa fa-info-circle'></i></a>";
                    if (usuario == item.usuario_recibio) {
                        filas += "<a href='" + "#caja_chica_modal" +
                            "' data-toggle='modal' data-proyecto='" + item
                            .tbl_proyectos_idtbl_proyectos + "' data-uid='" + item.uid +
                            "' data-action='new' title='Comprobar'><i class='fa fa-check-square-o'></i></a>";
                    } else if (item.estatus == 'finalizada' && tipo == 16) {
                        filas += "<a href='" + "<?= base_url() ?>" +
                            "almacen/comprobaciones_solicitud/" + item.uid + "' data-proyecto='" +
                            item.tbl_proyectos_idtbl_proyectos + "' data-uid='" + item.uid +
                            "' data-action='new' title='Comprobaciones'><i class='fa fa-eye'></i></a>";
                    }
                    filas += "</td>"
                    filas += "</tr>";
                }
            });
            $('#tbviaticos tbody').html(filas);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response.cajaschicas, function(key, item) {
                ...
                filas += "<tr class='" + clase + "'>";
                filas += "<td><a href='" + "<?= base_url() ?>" +
                    "almacen/comprobaciones_personal/" + item.uid + "' data-proyecto='" + item
                    .tbl_proyectos_idtbl_proyectos + "' data-uid='" + item.uid +
                    "' data-action='new' title='Comprobaciones'>" + item.uid + "</a></td>";
                filas += "<td><a href='" + "<?= base_url() ?>" +
                    "almacen/comprobaciones_personal/" + item.uid + "' data-proyecto='" + item
                    .tbl_proyectos_idtbl_proyectos + "' data-uid='" + item.uid +
                    "' data-action='new' title='Comprobaciones'>" + item.nombres + ' ' + item
                    .apellido_paterno + ' ' + item.apellido_materno + "</a></td>";
