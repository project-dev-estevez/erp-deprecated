      ...
      <div class="card-body">
        ...
        <?php echo form_open_multipart(base_url().'compras/enviar-solicitud', 'class="needs-validation" id="formuploadajax" novalidate'); ?>
          ...
          <!--<div class="row">
            ...
            <input type="hidden" class="form-control" value="<?= $uid ?>" name="recibe">
          <!--</div>-->
          <!--
          <div class="form-row">
            <div class="col">
  ...
  $(document).ready(function () {
    $("#formuploadajax").on("submit", function (event) {
      if (event.isDefaultPrevented()) {
        ...
        $.ajax({
          url: "<?= base_url()?>almacen/guardar_hoja_asignacion",
          type: "post",
          dataType: "json",
          data: formData,
          cache: false,
          ...
          complete: function (res) {
            ...
            if (resp.error == false) {
              ...
              window.location.replace("<?= base_url()?>contratistas/detalle-usuario/<?= $uid ?>");
            } else {
              button.prop("disabled", false);
              Toast.fire({
                type: 'error',
