  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header d-flex align-items-center">
        <h3 class="h4">Nueva Solicitud de Almacen(Alto Costo)</h3>
      </div>
      <div class="card-body">
        <div class="over"></div>
        ...
        <?php echo form_open_multipart(base_url().'compras/enviar-solicitud', 'class="needs-validation" id="formuploadajax" novalidate'); ?>
          ...
          <input type="hidden" name="tipo_producto" value="Alto costo">
          <hr>
...
<script>
  ...
  $("body").on('keyup', ".neodata", function(event) {
    ...
    var dataString = 'neodata=' + neodata + '&tipo=alto_costo';
    var proyecto = $("#selectProyecto").val();
    var segmento = $("#ssegmento").val();
    if(proyecto == null || segmento == null){
      $(this).val("");
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            $.each(data, function(key, item) {
                filas += "<div><a class='elemento-sugerido list-group-item' data-idtblcatalogo='" + item.idtbl_catalogo + "' data='" + item.neodata + " " + item.marca + " " + item.modelo + " (" + item.descripcion.substr(0,60) + ")" + "' data-precio='" + item.precio + "' data-unidad_medida='" + item.unidad_medida + "' data-moneda='" + item.tipo_moneda +"' data-preciolabel='" + parseFloat(item.precio).toFixed(2) + (item.tipo_moneda == "p" ? ' Pesos' : ' Dolares') + "' data-categoria='" + item.ctl_categorias_idctl_categorias + "'>" + item.neodata + " " + item.marca + " " + item.modelo + " (" + item.descripcion.substr(0,60) + ")" + "</a></div>";
            });
