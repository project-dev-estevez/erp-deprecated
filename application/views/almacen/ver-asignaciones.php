              ...
              <table class="table table-striped table-sm table-hover" id="tbasignaciones">
                <thead>
                  <tr>
                    ...
                    <th data-priority="2">Fecha asignación</th>
...
<script>
  ...
  function mostrarDatos(valorBuscar,pagina,valorBuscar2) {
    $.ajax({
      url: "<?= base_url() ?>Almacen/mostrarAsignaciones",
      type: "POST",
      data: {
        buscar:valorBuscar,
        nropagina:pagina,
      ...
      success:function(response) {
        filas = "";
        $.each(response.asignaciones,function(key,item) {
          if(item.estatus_movimiento=='0'){
            var estatus = 'Pendiente';
            var clase = '';                    
          } else if(item.estatus_movimiento == '1'){
            var estatus = 'Finalizada';
            var clase = '';
          }else{
          ...
          filas += "<td>";
          if (item.estatus_movimiento=='0') {
            filas += "<a href='" + "<?= base_url() ?>almacen/asignacion/continuar/" + item.uid_movimiento + "/" + item.uid_user + "' title=Continuar><i class='fa fa-edit'></i></a>";
          }
          else if(item.estatus_movimiento=='1') {
            filas += "<a href='" + "<?= base_url() ?>almacen/asignacion/detalle/" + item.uid_movimiento + "' title=Detalles  onClick=\"" + "window.open(this.href, this.target, 'width=1000,height=800,left=0,top=0'); return false;" + '"' + "><i class='fa fa-eye'></i></a>";
          }
          filas += "</td>";
          filas += "</tr>";
        });
