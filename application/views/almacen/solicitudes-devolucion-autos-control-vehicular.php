          ...
          <div class="bg-white has-shadow">
            <a href="<?= base_url() ?>almacen/solicitud-devolucion-autos-control-vehicular" class="dropdown-item" title="">
              <div class="item d-flex align-items-center pr-4 pl-4">
                ...
                <div class="title">
                  <span>Nueva Solicitud<br> de Devolución Autos<br> Control Vehicular</span>
                </div>
              </div>
            </a>
          </div>
  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header d-flex align-items-center">
        <h3 class="h4">Mis solicitudes de devolución autos control vehicular</h3>
      </div>
      <div class="card-body">
        <div class="float-right">
          <input type="text" class="form-control" placeholder="Buscar" name="busqueda">
        </div>
        <!--<button onclick="window.location.href='<?php echo base_url() ?>almacen/excel-devoluciones-AC'" class="btn btn-secondary buttons-excel buttons-html5 btn-info" tabindex="0" aria-controls="data-table"><span><i class="fa fa-file-excel-o"></i> Exportar a Excel</span></button>-->       
        <div class="table-responsive">
          <table class="table table-striped table-sm table-hover" id="tbdevolucionesacv">
            <thead>
              <tr>
                <th>UID</th>
                <th>Creación</th>
...
<script>
  $(document).ready(function(){
    ...
    $("input[name='busqueda']").keyup(function() {
      ...
      textoBuscar2 = $("select[name='selectACV']").val();
  ...
  function mostrarDatos(valorBuscar,pagina,valorBuscar2) {
    $.ajax({
      ...
      success:function(response) {
        ...
        $('#tbdevolucionesacv tbody').html(filas);
