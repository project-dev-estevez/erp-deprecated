  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header d-flex align-items-center">
        <h3 class="h4">Nueva Solicitud de Refacciones(Control Vehicular)</h3>
      </div>
      <div class="card-body">
        <div class="over"></div>
        ...
        <?php echo form_open_multipart(base_url().'compras/enviar-solicitud', 'class="needs-validation" id="formuploadajax" onsubmit="return checkSubmit();" novalidate'); ?>
          ...
          <?php } else{ ?>
            <input type="hidden" name="eco" value="<?= $iddtl_almacen ?>">
          <?php } ?>
          <input type="hidden" name="tipo_producto" value="Refacciones Control Vehicular">
          <hr>
          ...
          <div class="clearfix pt-md">
            <div class="pull-right">
              ...
              <?= form_hidden('idtbl_tramites_vehiculares', $idtbl_tramites_vehiculares) ?>
        ...
        if (_this.val() == 'interno') {
          ...
          $.each(personalAutoriza, function (i, item) {
            <?php if($this->session->userdata('tipo') == 15){ ?>
              console.log(item.idtbl_users);
              if(item.idtbl_users == 66 || item.idtbl_users == 157){
        ...
        $.ajax({
          ...
          complete: function (res) {
            ...
            if (resp.error == false) {
              ...
              <?php if($uid_tramites_vehiculares == ""){ ?>
                window.location.replace("<?= base_url()?>almacen/solicitudes-almacen");
              <?php }else{ ?>
                window.location.replace("<?= base_url()?>control-vehicular/detalle-servicio/<?= $uid_tramites_vehiculares ?>");
              <?php } ?>
            } else {
