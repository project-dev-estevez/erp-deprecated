...
<script>
  ...
  function mostrarDatos(valorBuscar, pagina) {
    ...
    $.ajax({
      url: "<?= base_url() ?>Empresas/mostrarDepartamentosEstablecimientos",
      type: "POST",
      data: {
        buscar: valorBuscar,
        nropagina: pagina,
      ...
      success: function(response) {
        ...
        $.each(response.departamentos, function(key, item) {
          filas += "<tr><td>" + item.uid + "</td><td>" + "<a href='<?= base_url() ?>departamentos/departamento/" + item.uid + "'>" + item.departamento + "</a>" + "</td><td>" + item.perfiles + "</td><td>" + item.personal + "<td><a href='#editar_departamento' data-toggle='modal' title='" + item.departamento + "'" + " data-departamento='" + item.departamento + "'" + " data-uid='" + item.uid + "'>" + "Editar" + "</a></td>" +"<//tr>";
        });
