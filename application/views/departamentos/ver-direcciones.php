...
<section class="tables">
  ...
  <!-- nueva tabla-->
  <input type="hidden" id="establecimiento" value="<?= $id_establecimiento; ?>">
  <div class="container-fluid">
    <div class="card">
      <div class="card-header d-flex align-items-center">
        <h3 class="h4">Direcciones</h3>
      </div>
      <div class="card-body">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <input type="text" placeholder="Departamento" required="required" minlength="4" maxlength="50" name="departamento" class="form-control capitalize">
        </div>
      </div>
...
<script>
  ...
  function mostrarDatos(valorBuscar, pagina) {
    ...
    $.ajax({
      url: "<?= base_url() ?>Empresas/mostrarDirecciones",
      type: "POST",
      data: {
        buscar: valorBuscar,
        nropagina: pagina,
      ...
      success: function(response) {
        filas = "";
        $.each(response.direcciones, function(key, item) {
            filas += "<tr><td>" + item.uid + "</td><td>" + "<a href='<?= base_url() ?>empresas/areas/"+ item.idtbl_direcciones + "'>" + item.direccion + "</a>" + "</td>" + "</tr>";
        });
