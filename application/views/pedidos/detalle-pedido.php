        ...
        <div class="table-responsive">
          <table class="table table-border">
            ...
            <tbody>
              ...
              <?php endforeach ?>
              <tr>
                <td colspan="5" rowspan="<?= $pedido->porcentaje_retencion != NULL || $pedido->amortizacion != NULL ? strval(7) : strval(6) ?>"><strong>Nota del pedido:</strong> <?= $pedido->observaciones ?></td>
                <th>SUBTOTAL</th>
              ...
              <?php
                  ...
                  if($pedido->iva_retencion !== 0 && $pedido->iva_retencion !== NULL){
                    $iva_retencion_total = $pedido->iva_retencion/100;
                    $total = $total - ($subtotal*$iva_retencion_total);
                  }
                ?>
              ...
              <?php if($pedido->iva_retencion != NULL){ ?>
                ...
                <th>IVA <?= $pedido->iva_retencion ?>%</th>
                  <?php $iva_retencion = $pedido->iva_retencion/100 ?>
                  <td>$<?= number_format($subtotal*$iva_retencion,2) ?></td>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h3 class="h4">Nueva Entrada - Almacen (<?= $pedido->almacen ?>)</h3> 
            </div> 
            <div class="card-body">
                        ...
                        <tr class="resultado">
                            ...
                            <?php $entregados = 0; $pendientes = 0; ?>
                                    ...
                                    <?php foreach($data['detalle'] AS $key => $value){ ?>
                                      <?php if($value->cantidad != $value->entregado){ ?>
                                        <tr class="tr">
                                          ...
                                          <?php if($value->tbl_catalogo_idtbl_catalogo_pedido == NULL){ ?>
                                <td style="width: 180px;"><center><label><strong>Neodata Solicitado </strong><i style=font-size:18px; class="fa fa-question-circle fa-2xs"data-toggle="tooltip" data-placement="top" title="Cambiar código al homologado"></i>
                                </button></label></center><center><select style="padding:0px;" class="form-control idcatalogo" name="id-producto-catalogo[]" id="s_<?= $value->tbl_catalogo_idtbl_catalogo ?>_<?= $pendientes ?>"><option value="<?= $value->tbl_catalogo_idtbl_catalogo ?>"><?= $value->neodata ?></option></select></center></td>
                                <!--<td style="width: 180px;"><center><label><strong>Neodata Pedido </strong><i style=font-size:18px; class="fa fa-question-circle fa-2xs"data-toggle="tooltip" data-placement="top" title="Cambiar código al homologado"></i>
                                </button></label></center><center><select style="padding:0px;" class="form-control idcatalogo" name="id-producto-catalogo[]" id="s_<?= $value->tbl_catalogo_idtbl_catalogo_pedido ?>_<?= $pendientes ?>"><option value="<?= $value->tbl_catalogo_idtbl_catalogo_pedido ?>"><?= $value->neodata_pedido ?></option></select></center></td>-->
                                <?php }else{ ?>
                                <td style="width: 180px;"><center><label><strong>Neodata Solicitado </strong><i style=font-size:18px; class="fa fa-question-circle fa-2xs"data-toggle="tooltip" data-placement="top" title="Cambiar código al homologado"></i>
                                </button></label></center><center><?= $value->neodata ?></center><center><input type="radio" id="html" name="id-producto-catalogo[<?= $pendientes ?>]" value="<?= $value->tbl_catalogo_idtbl_catalogo ?>"></center></td>
                                <td style="width: 180px;"><center><label><strong>Código Pedido Neodata </strong><i style=font-size:18px; class="fa fa-question-circle fa-2xs"data-toggle="tooltip" data-placement="top" title="Cambiar código al homologado"></i>
                                </button></label></center><center><?= $value->neodata_pedido ?></center><center><input type="radio" id="html" name="id-producto-catalogo[<?= $pendientes ?>]" value="<?= $value->tbl_catalogo_idtbl_catalogo_pedido ?>"></center></td>
                                <?php } ?>
                                ...
                                <center><label style="color: red;"><strong>Número interno*</strong></label></center>
                                 <?php
                                  //$letras = preg_replace('/[^A-Za-z]/', '', $value->ultimo_numero_interno);
                                  //$numeros = preg_replace('/[^0-9]/', '', $value->ultimo_numero_interno);
                                  //$nuevo_numero = (int)$numeros ;
                                  //$contador = isset($contador) ? $contador + 1 : 1; // Variable de contador para saber cuántas veces se ha repetido el valor
                                  //$nuevo_numero_interno = $letras . $nuevo_numero + $contador;
                                  ?>
                                  <input type="text" name="numero_interno[]" class="form-control" value="">                                </td>                                                               
                                </tr>
                                
                                <?php if($this->session->userdata('tipo') == 4): ?>
                                <tr>
                                  <td colspan="4">
                                    <center><label for="racks"><strong>Rack</strong></label></center>
                                    <select name="rack[]" class="form-control racks_1" id="racks" nivel="<?= $key ?>">
                                  ...
                                  <td colspan="4">
                                    <label>Nivel</label>
                                      <select name="nivel[]" class="form-control" id="nivel_<?= $key ?>"><option value="" disabled="disabled" <?php if ($result->tbl_rack_idtbl_rack==NULL): ?>selected="selected"<?php endif ?>>Seleccione...</option></select>
                                  </td>
                                </tr>
                                <?php endif; ?>
                      
                                <tr class="resultado">
                                <td id="td_tipo_documento" colspan="4">
                                ...
                                <input type="text" name="numero_documento[]" class="mayusculas form-control" requiredg
                                ...
                                <tr class="tr">
                                <td colspan="4"><center><strong>Proveedor</strong></center></td>
                                <td colspan="3"><center><strong>Lugar de Entrega</strong></center></td>
                                <td colspan="2"><center><strong>Fecha de Entrega</strong></center></td>
                                </tr>
                                <tr class="tr">
                                <td colspan="4"><center><?= $pedido->nombre_fiscal ?></center></td>
                                <td colspan="3"><center><?= $pedido->lugar_entrega ?></center></td>
                                <td colspan="2"><center><?= $pedido->fecha_entrega ?></center></td>
                                </tr>
                                <tr>
                                <td style="background-color:gray;" colspan="9"></td>
                                </tr>
                                <?php $pendientes++; ?>
                                      <?php }if($pedido->cantidad == $pedido->entregado){ ?>
                                        <tr class="tr">
...
$(document).ready(function(){
  var option = '';
  $(document).on('change', '.racks_1', function(){
    var nivel=$(this).attr('nivel');
    console.log('#nivel_'+nivel);
    $('#nivel_'+nivel).html('').append('<option value="" disabled="disabled" selected="selected">Seleccione...</option>');
    $.ajax({
      ...
      data: {racks: $(this).val()},
      beforeSend: function() {
        $(this).closest('.card-body').addClass('load');
        },
    ...
    .always(function() {
      $(this).closest('.card-body').removeClass('load');
    });
  });
});
</script>
